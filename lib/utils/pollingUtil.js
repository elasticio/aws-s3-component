  ...
  async emitIndividually(files) {
    ...
    for (let i = 0; i < files.length; i += 1) {
      ...
      try {
        ...
        if (this.cfg.enableFileAttachments && this.cfg.enableFileAttachments === 'true') {
  ...
  async emitAll(results) {
    ...
    if (this.cfg.enableFileAttachments && this.cfg.enableFileAttachments === 'true') {
      ...
      for (let i = 0; i < results.length; i += 1) {
        ...
        await this.s3FileToAttachment(dummyMessage, file.Key)
        attachments[file.Key] = dummyMessage.attachments[file.Key];
      }
