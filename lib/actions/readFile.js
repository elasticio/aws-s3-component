...
exports.process = async function (msg, cfg) {
  ...
  if (result.Size > params.ATTACHMENT_MAX_SIZE) {
    this.logger.error('File %s with size %d bytes is too big for attachment usage', result.Key, result.Size);
    throw new Error(`File ${result.Key} with size ${result.Size} bytes is too big for attachment usage`);
  }
